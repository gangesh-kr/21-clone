...
export default function MountainScene() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        // ── GIF parallax: scale up slightly, drift up, fade out ──
        if (gifRef.current) {
          // Scale from 1.08 → 1.25 as you scroll (slow zoom-in feel)
          const scale = 1.08 + heroPercent * 0.17

          // Translate upward — mountain rises and exits top
          const translateY = heroPercent * -12 // percent units on a 100vh container

          // Opacity: hold full until 15% scroll, then fade to 0 at 75%
          const opacity = heroPercent < 0.15
            ? 1
            : Math.max(0, 1 - (heroPercent - 0.15) / 0.6)

          gifRef.current.style.transform = `scale(${scale}) translateY(${translateY}%)`
          gifRef.current.style.opacity = opacity
        }
  ...
  return (
    <div
      ref={wrapperRef}
      className="fixed inset-0 w-full h-full pointer-events-none z-0 overflow-hidden bg-[#cbd3db]"
    >
      {/*
        The GIF is positioned to fill the viewport.
        object-fit: cover mirrors how the WebGL canvas filled the screen.
        transform-origin: center bottom keeps the mountain base anchored
        while the peak scales upward — mimicking a cinematic zoom.
      */}
      <img
        ref={gifRef}
        src="/hero-gif.gif"
        ...
        style={{
          ...
          // Subtle color correction to match the existing glacier-gray palette
          filter: 'saturate(0.85) brightness(1.02)',
        }}
      />

      {/*
        Gradient vignette at the bottom — blends the GIF edge into the
        dark section backgrounds below, same role as the fog in the WebGL version.
      */}
      <div
        style={{
          position: 'absolute',
          ...
          right: 0,
          height: '35%',
          background: 'linear-gradient(to bottom, transparent, var(--color-charcoal-navy, #0a0e17))',
          pointerEvents: 'none',
        }}
      />

      {/*
        Side vignettes — subtle darkening of edges like the WebGL Vignette postprocess.
      */}
      <div
        style={{
          position: 'absolute',
